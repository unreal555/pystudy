...
filename='新浪.csv'
logname='log.txt'
abs_path=os.path.join(base_dir,today)
data_file_path=os.path.join(base_dir,today,filename)
log_file_path=os.path.join(base_dir,today,logname)
...
columns_names=['url','title','content','riqi','pic']

start = '''http://mil.news.sina.com.cn/roll/index.d.html?cid=57918'''
...
def get_news_index(start):

    page=my_request(url=start,headers=headers,debug=False)
...
def Main():
    news_index=get_news_index(start=start)
