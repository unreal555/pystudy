...
async def main(username, pwd, url):# 定义main协程函数，
    ...
    browser = await launch({'headless': False ,'args':['--window-size=1366,850']}) # 启动pyppeteer 属于内存中实现交互的模拟器  , 'args': ['--no-sandbox']
    ...
    if slider:
        ...
        if flag:
            ...
            print(cookies_list)
            cookies_list=await get_cookie(page) # 导出cookie 完成登陆后就可以拿着cookie玩各种各样的事情了。
            return cookies_list,page

    else:
        print("无滑块")
        # await page.keyboard.press('Enter')
        # print("print enter")
        await page.evaluate('''document.querySelector("#J_SubmitStatic").click()''')
        ...
        try:
            ...
            if error:
                ...
                print(page.url)
                cookies_list=await get_cookie(page)
                # time.sleep(100)
                return cookies_list,page
...
async def get_page(page,url):
    ...
    await page.type('#J_TSearchForm', '小悦家', {'delay': input_time_random()})
    time.sleep(2)
...
if __name__ == '__main__':
    ...
    print('aaaaa',cookies,page)
    loop.run_until_complete(get_page(page,'https://www.taobao.com'))
